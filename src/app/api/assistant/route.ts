...
export async function POST(request: NextRequest) {
    try {
        ...
        // Sport-specific system context
        const baseballContext = `You are a cutthroat expert fantasy baseball analyst playing in a massively deep 18-team, 7x7 categories league (R, H, HR, RBI, SB, AVG, OPS x W, SV, K, HLD, ERA, WHIP, QS).
...
You MUST use your Google Search tool to verify the most recent 2026 spring training news, injury statuses, and positional changes before recommending any player. Real-time accuracy is critical.`;
        ...
        const basketballContext = `You are a cutthroat expert fantasy basketball analyst. This is a roto/categories league.
...
=== IMPORTANT GROUNDING RULE ===
You MUST use your Google Search tool to verify the most recent injury reports, lineup changes, and player news before making recommendations. Real-time accuracy is critical.`;

        const sportContext = sport === 'basketball' ? basketballContext : baseballContext;

        const systemContext = `${sportContext}
...
${picksUntilTurn !== undefined ? `Picks until my next turn: ${picksUntilTurn}` : ''}

=== AVAILABLE PLAYERS (top 150 by ADP/AR) ===
${topBoardClipped}

=== INSTRUCTIONS ===
Analyze using your expert knowledge of ${sport === 'basketball' ? 'NBA players, schedules, and roto categories' : 'MLB prospects, playing time, and 7x7 category formats'}.
${customPrompt ? `\nCRITICAL USER REQUEST — MUST prioritize: "${customPrompt}"\n` : ''}

If the user is asking for player recommendations, add/drop advice, or trade analysis, respond with a JSON array:
[
  {
    "name": "Player Name",
    "pos": "Position String",
    ...
    "rationale": "2-3 sentences on why they fit this roster right now."
  }
]

If the user is asking a general question about players, injuries, news, or strategy, respond conversationally in plain text. You can still provide insights and analysis, just don't force it into JSON format.`;
